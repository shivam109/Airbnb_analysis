...
client = pymongo.MongoClient(
    "mongodb+srv://shivam109kumar:CqrsUa9Fqiekp9gy@cluster0.tyzd6il.mongodb.net/?retryWrites=true&w=majority")
...
if selected == "Overview":
    ...
    with tab2:
        # GETTING USER INPUTS
        country = st.sidebar.multiselect('Select a Country', sorted(
            df.Country.unique()), sorted(df.Country.unique()))
        prop = st.sidebar.multiselect('Select Property_type', sorted(
            df.Property_type.unique()), sorted(df.Property_type.unique()))
        room = st.sidebar.multiselect('Select Room_type', sorted(
            df.Room_type.unique()), sorted(df.Room_type.unique()))
